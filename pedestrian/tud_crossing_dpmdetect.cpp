...
int main(int argc, char** argv) {
	...
	string input_dir = "/home/user/ccv/data/tud-crossing-sequence";
	...
	Mat getSize = imread(input_dir + "/DaSide0811-seq7-001.png");
	Size videoSize = getSize.size();
	getSize.release();
	VideoWriter outputVideo;
	cout<<"The video size is "<<videoSize.width<<" "<<videoSize.height<<endl;
	outputVideo.open("/home/user/ccv/data/output/no_red_tud_reg_0.8.avi", ex, fps, videoSize, true);
	...
	for (int iImage = imageStart; iImage <= imageEnd; iImage++) {
		...
		string imageTail;
		if (iImage < 10) imageTail = "00" + patch::to_string(iImage);
		else if (iImage < 100) imageTail = "0" + patch::to_string(iImage);
		else imageTail = patch::to_string(iImage);
		string image_name = input_dir + "/DaSide0811-seq7-" + imageTail + ".png";
		cout<<"Processing the image "<<image_name<<endl;
		...
		if (seq != NULL) { 
			...
			for (int i = 0; i < seq->rnum; i++) {
				...
				if (abs(diff) > predHeight * 0.5 && abs(diff) > comp->rect.height) {
